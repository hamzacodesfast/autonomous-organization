# AUTONOMOUS ORGANIZATION — BRAND SPEC v0.3
...
| Hermes agent (`https://github.com/nousresearch/hermes-agent`) | sole autonomous orchestration runtime, job scheduler, guarded executor, and escalation coordinator | Class 2 by default; Class 5 for runtime permissions, tool grants, or policy bypass |
...
| Pump.fun livestream and token surfaces | dormant public runtime and token-adjacent operations pending later human approval | Class 3 only for explicitly approved stream content; Class 5 for wallet, token, or financial authority |

### 0.3 Runtime principle

Hermes is the Organization's operating runtime. Drafting may happen in other tools, but recurring autonomous work and any machine-initiated Class 2 or higher action must run as a Hermes job or a Hermes-supervised subprocess. The goal is one accountable runtime, not many silent automations.

If a script, cron job, webhook consumer, or SaaS automation changes public state, inventory, fulfillment state, dashboard state, or sends external communication, that automation is considered part of Hermes and must inherit the same action class, approval, logging, and escalation rules. Standalone automations that cannot expose run IDs, approvals, logs, heartbeat, and kill-switch behavior are not production-ready.
...
- Hermes misses two expected heartbeat intervals, loses queue visibility, or reports a stuck worker
- Any Class 2 or higher Hermes job lacks a run ID, requests an unapproved surface, or attempts publication without a valid approval record when one is required
...
Hermes is the sole autonomous executor for the Organization. Hermes may call approved tools, scripts, APIs, and subprocesses, but those actions are treated as Hermes actions, not as free-standing automations.

Rules:

- Class 0 and Class 1 drafting may happen outside Hermes, but any recurring job or machine-initiated Class 2 or higher action must run through Hermes.
- Every Hermes job has a run ID, requested action class, target surface, retry budget, and linked Local, order, or asset IDs when relevant.
- Hermes refuses execution when the requested class exceeds scope, approval is missing or expired, the surface is unapproved, or a required secret or runtime flag is absent.
- Hermes maintains a visible heartbeat and operator-readable queue state.
- Hermes exposes an operator kill switch that halts new jobs and cancels in-flight publication or fund-committing jobs where rollback is possible.
- Emergency human actions outside Hermes are permitted only for Class 5 recovery or outage mitigation and must be entered into the action log immediately after the fact.

### 12.2 Permission classes
...
**Class 2 — Stage.** Hermes may create private products, unpublished pages, unsent scheduled post drafts, test-mode Stripe objects, Printify drafts, and staging records when the action is reversible and not visible to the public.

**Class 3 — Publish.** Hermes may publish externally visible material only when the relevant approval rule in this file has been satisfied. Public material includes product pages, live products, social posts, livestream scenes, emails to customers, scheduled posts, and changes to dashboard schema or copy. Pre-approved dashboard metrics may update automatically when the underlying fields are approved and sanitized.

**Class 4 — Commit funds.** Hermes may never commit funds above approved bands without human approval. This includes refunds above $100 USD, paid ad spend, supplier upgrades, software subscriptions, domain renewals, crypto transactions, or any irreversible payment.

**Class 5 — Root.** Hermes may never change root controls without human approval. Root controls include DNS, registrar settings, VPS firewall rules, production environment variables, API scopes, OAuth apps, payout accounts, tax settings, bank accounts, wallets, and this file.

### 12.3 Action logging

Every Hermes job or emergency human recovery action of Class 2 or higher writes an action log before and after execution. Logs are append-only. The log records:

- UTC timestamp
- agent or process name
- Hermes job or run ID
- action class
...
The "before" log is written before side effects occur. The "after" log is written immediately after completion, failure, or rollback. If Hermes cannot write the after-log, it halts further Class 2 or higher execution on the affected surface and alerts `#alerts`.
...
Hermes validates approval scope before execution. A Class 3 job with missing, expired, or out-of-scope approval fails closed. Recurring schedules may queue draft publication jobs in advance, but they may not execute after approval expiry or after a material change to the approved object.
...
Hermes may read registry metadata to decide whether an action is permitted, but the registry is never a secret source.
...
Within 72 hours of each release, Hermes prepares a short postmortem covering launch timing, inventory movement, fulfillment errors, customer issues, revenue, gross margin, social performance, and recommended changes. The postmortem is internal unless the human approves a public summary.
...
Hermes is the only autonomous controller permitted to switch public scenes, update livestream overlays, or publish sanitized runtime summaries.
...
- Hermes scheduler, heartbeat, queue visibility, and operator kill switch have been tested.
- At least one reversible Class 2 Hermes job, one blocked Class 3 Hermes job, and one rollback path have been exercised with correct approval and action-log behavior.
- Discord alert routes, approval records, action logs, and rollback procedures have been tested.
- Support inbox ingestion, escalation routing, and operator notification have been tested.
- Secret scanning is clean for Git, environment files, logs, screenshots, and livestream scenes.
- Token registry metadata is current for every production credential in use.
...
- Whether a routine Hermes job may execute when approvals, templates, and safety rules are satisfied
...
- Whether to halt and escalate under § 11
...
- 2026-04-20 — v0.3 — Human-requested Hermes-centered operating model. Defined Hermes as the sole autonomous runtime, added queue, heartbeat, kill-switch, approval validation, action-log enforcement, support-ingestion gate, and token-registry/runtime requirements.
